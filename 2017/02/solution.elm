...
lineCheck : List Int -> Int
lineCheck list =
    (List.maximum list |> Maybe.withDefault 0) - (List.minimum list |> Maybe.withDefault 0)


lineCheck2 : List Int -> Int
lineCheck2 =
    let
        go list =
            case list of
                head :: tail ->
                    case List.filter (\x -> max head x % min head x == 0) tail of
                        [] ->
                            go tail

                        head2 :: _ ->
                            max head head2 // min head head2
...
x =
    Debug.log "solution" (solution input)


y =
    Debug.log "solution2" (solution2 input)
