...
-- A light fork of the AdventOfCode.ArrowParser module at the time of this
-- puzzle.  The primary diffenence is that the expected character is in the
-- error (or impurely undefined), and it's LL1 (via no backtracking once on
-- option matches at all).
--
-- NOTE: DOCUMENTATION IS ALL SUSPECT DUE TO THESE CHANGES
  ...
  (runAP -> f) <+> (runAP -> g) =
    AP
      ( \x@(_, i, _) ->
          case f x of
            Right x -> Right x
            Left e@(ei, _) -> if ei == i then g x else Left e
      )
  ...
  AP
    ( \(s, i, _) ->
        case Seq.lookup i s of
          Nothing -> Left (i, UnexpectedEndOfInput undefined)
          Just t ->
            if f t
              then Right (i + 1, t)
...
token t =
  AP
    ( \(s, i, _) ->
        case Seq.lookup i s of
          Nothing -> Left (i, UnexpectedEndOfInput t)
          Just f ->
            if f == t
              then Right (i + 1, t)
              else Left (i, UnexpectedToken f)
    )
